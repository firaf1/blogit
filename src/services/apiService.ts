          ...
          if (category.blogs && category.blogs.data && Array.isArray(category.blogs.data)) {
            category.blogs.data.forEach((blog: any) => {
              allBlogs.push({
                ...
                summary: blog.description ? blog.description.substring(0, 150) + "..." : "No description",
                ...
                imageUrl: blog.background_image || `https://picsum.photos/seed/${blog.id}/800/450`,
                isSaved: false,
  ...
  async getBlogDetail(id: number | string): Promise<NewsItem | null> {
    try {
      ...
      return {
        id: blog.id,
        title: blog.title,
        summary: blog.description || "No summary available",
